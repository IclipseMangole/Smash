...
import static de.IM.Smash.Util.TypeUtils.splitStringArrayBy;
...
public class BukkitCommand extends Command implements CommandExecutor {
    ...
    public BukkitCommand(JavaPlugin plugin, Object function, Method method, IMCommand baseCommand) {
        super(baseCommand.name(), splitStringArrayBy(".", baseCommand.parent()) + baseCommand.name() + ".description", baseCommand.name() + ".usage", Arrays.asList(baseCommand.aliases()));
        this.plugin = plugin;
        this.function = function;
        this.command = baseCommand;
        this.processor = new GlobalCommand<CommandSender>(plugin, baseCommand, function, method, Player.class) {
            @Override
    ...
    public void register() {
        ...
        if (cmd != null) {
            String usage = command.name() + ".usage";
            if (usage.isEmpty()) {
                usage = cmd.getUsage();
            }
